...
flags.DEFINE_string("output_dir", "tfrecords", "The directory of tfrecord output [tfrecords]")
...
def persistence_image_data_to_tfrecords(x_data, y_data, data_type, split_index=128):
    ...
    for images_filename, y_label in zip(x_data, y_data):
        ...
        try:
            ...
            grayscale_image = tf.image.rgb_to_grayscale(tf.image.rgb_to_grayscale(image))
            coordi = iputil.get_bounding_size_info(y_label, images_filename)
            cropped = tf.image.crop_to_bounding_box(grayscale_image, coordi[0][1], coordi[0][0], coordi[0][3], coordi[0][2])
            image = tf.image.resize_images(cropped, [32, 32])
        ...
        image_list = [flatten_image]

        for image in image_list:
            try:
                image_bytes = sess.run(tf.cast(image, tf.uint8))
...
def main(_):
    print('Converting dog image to tfrecord datatype')
